...
// Errors from blocked external resources (the jsdelivr basis transcoder CDN, a
// blueprint host's firebase backend, etc.) are environment noise in the sandbox,
// not app bugs — filter them so assertions only see app/page errors.
const isAppError = (line: string): boolean =>
    !/net::ERR_|Failed to load resource|jsdelivr|firebaseio|googleapis/i.test(line)

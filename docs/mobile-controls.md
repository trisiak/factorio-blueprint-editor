...
- **Compatibility** — `inputMode.mode` survives as a _derived_ value (forced
  ...
  every consumer that hasn't migrated yet keeps today's behaviour. It goes away
  when the last consumer moves onto the signals (#101, later slices).
  `armMarquee` left that list in Slice 2 — it no longer gates on the mode at all
  — the **contextual clusters** left it in Slice 3 (they read `coarse` / `keys` /
  `touchRecent` and the editor mode, nothing else), the action rail and the top
  band in Slice 4, and the Pixi panels retired in Slice 5. What still reads
  `.mode` is the dialog layer (the DOM-vs-Pixi dialog gates in `UIContainer`,
  `inventorySelector.ts`, `shell.ts`, the Pixi `InventoryDialog`'s commit/hover
  rules and the editors' clear hint — Slice 6) and `body.mobile` via
  `settingsPane.ts`.

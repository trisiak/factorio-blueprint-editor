...
| **Entity-info sheet** (`#entity-info-sheet`)     | inside `#readout-stack` when wide (**top-right**); `compact` portrait: **top band**, landscape: **bottom-right**               | wide: `min(320px,34vw)` × ≤40vh; compact portrait: full width minus the rail; scrolls   | 15      | **Universal** since #101 Slice 5 (was mobile-only). Passive readout — tier 2 of the layering contract: hides while a Pixi dialog is open. Placement by `compact` + orientation, never by input mode; icons via `packIcons.ts`                                                                                                                                                                                                                                                                                                                                               |
| **Rates drawer** (`#rates-drawer`)               | inside `#readout-stack` when wide (**right, below the sheet**); `compact` portrait: **bottom-right**, landscape: **top-right** | wide: the stack's width × ≤`min(50vh,420px)`; compact: `min(300px,70vw)`; scrolls       | 15      | **Universal** since #101 Slice 5. Explicitly toggled (`T` / rail "Rates" / its own ✕), so in compact portrait it takes the _reachable_ bottom band while the passive sheet goes top — the reachability rule. Same tier-2 dialog gate                                                                                                                                                                                                                                                                                                                                        |
| **Readout stack** (`#readout-stack`)             | **right edge**, below the top band                                                                                             | wrapper: `min(320px,34vw)` wide, `pointer-events:none` except its children              | 15      | Layout box only (#101 Slice 5): the two readouts stack in a flex column when wide — a fixed box can't sit under one whose height varies with the recipe. On `body.compact` it becomes `display: contents` and each readout resumes its own fixed placement                                                                                                                                                                                                                                                                                                                  |
...
4. ✅ **Phase 2 — reclassify by role**: actions → rail; **entity-info →
   ...
   **placement by `compact` + orientation** rather than by mode: a shared
   right-edge column (`readoutStack.ts`) when wide, the portrait/landscape touch
   placements when `compact`. The editor keeps the projections only
